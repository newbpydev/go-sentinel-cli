...
// Test 1.1.1: Define TestResult structure with required fields for Vitest-like display
func TestTestResultStructure(t *testing.T) {
	...
	if result.Status != StatusPassed {
		t.Errorf("Expected Status to be '%s', got '%s'", StatusPassed, result.Status)
	}
...
func TestTestSuiteStructure(t *testing.T) {
	...
	test1 := &TestResult{Name: "Test1", Status: StatusPassed, Duration: 50 * time.Millisecond}
	test2 := &TestResult{Name: "Test2", Status: StatusFailed, Duration: 100 * time.Millisecond}
	...
	if suite.Tests[0].Name != "Test1" || suite.Tests[0].Status != StatusPassed {
		t.Errorf("First test not as expected")
	}

	if suite.Tests[1].Name != "Test2" || suite.Tests[1].Status != StatusFailed {
...
func TestFailedTestDetailStructure(t *testing.T) {
	...
	if detail.Result.Status != StatusFailed {
