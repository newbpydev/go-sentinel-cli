...
`
	...
	// Find tests by name
	var testExample, testFailing *models.LegacyTestResult
	...
	if testExample.Status != models.TestStatusPassed {
	...
	if testFailing.Status != models.TestStatusFailed {
...
`
	...
	pkg1 := findTestPackage(results, "github.com/user/project/pkg1")
	if pkg1 == nil {
	...
	if test1.Status != models.TestStatusFailed {
	...
	pkg2 := findTestPackage(results, "github.com/user/project/pkg2")
	if pkg2 == nil {
	...
	pkg3 := findTestPackage(results, "github.com/user/project/pkg3")
	if pkg3 == nil {
	...
	if test3.Status != models.TestStatusFailed {
	...
	if test3.Error == nil {
		t.Fatal("Expected error to be non-nil for timeout test")
	}

	if !strings.Contains(test3.Error.Message, "test timed out") {
		t.Errorf("Expected timeout error message, got '%s'", test3.Error.Message)
	}
}

// Helper function to find a package by name
func findTestPackage(packages []*models.TestPackage, name string) *models.TestPackage {
